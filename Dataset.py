...
class Dataset:
    ...
    def basicInfo(self):  
        ...
        '''
        dfBasic = (self.getDf().dtypes.reset_index()) # reset_index returns a DF with two columns and not a Series
    ...
    def unassembledSequence(self):
        ...
        unassembled = entire[entire['type'] == 'supercontig']  # selects from these rows only the ones of type supercontig
        fraction = str(unassembled.shape[0]) + "/" + str(entire.shape[0])  # fraction of supercontig content over all dataframe content
        perc = round(float(unassembled.shape[0]) * 100 / float(entire.shape[0]),2)  # percentage rounded on 2 decimal
        values = [fraction, perc]
        return Dataset(pd.DataFrame([values], columns=["fraction", "percentage %"]))

    @active
    def onlyEnsemblHavana(self):
        '''
    ...
    def entriesEnsemblHavana(self):  
        ...
        return filteredDs.countType() #using the method instantiated before for counting the Type in a Dataset
    ...
    def ensemblHavanaGenes(self):  
        ...
        for index, row in genes.iterrows():
            ...
            attDict = {}
            for i in attList:
                (key, value) = i.split("=") # we split the elements of the attList by "="
                attDict[key] = value # we add the elements in the dictionary

            geneNames.append(attDict['Name']) # we add the gene name in the geneNames list
