...
class Dataset:
    ...
    def getDf(self):
        """
         Retrieves the private dataframe attribute
        """
    ...
    def __uniqueCount(self,
                      column):  # private method that given the column label returns the count of each one of its values
    ...
    def entireChromosome(
            self):  # returns a new dataframe selecting from the column source only the lines having GRCh38 as source
    ...
    def unassembledSequence(
            self):  # returns the fraction and percentage of unassembled sequences over entire chromosomes set
    ...
    def onlyEnsemblHavana(
            self):  # returns a new dataframe selecting only the lines having ensembl, havana and ensembl_havana as source
